...
async function main() {
    ...
    //setup twitter user
    var user = new TwitterUser_1.TwitterUser();
    ...
    if (process.argv[2] === '-rebuildFollowers') {
        ...
        var followers = await user.GetFollowersForUser(screen_name, { forceRebuild: true });
        console.log(`Follower cache for ${user.GetScreenName()} rebuilt`);
        return;
    }
    ...
    var campaign = MessagingCampaign_1.MessagingCampaign.fromJSON(json);
    if (campaign) {
        var campaignManager = new MessagingCampaign_1.MessagingCampaignManager(user, campaign);
        campaignManager.Run();
    }
}
main();
